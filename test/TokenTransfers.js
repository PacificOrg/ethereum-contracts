...
contract('TokenTransfers', function(accounts) {
  ...
  it('check sending tokens', async() => {
    ...
    assert.sameMembers([(await tokenInstance.balanceOf.call(accounts[1])).toString(), (await tokenInstance.balanceOf.call(accounts[2])).toString(), (await tokenInstance.balanceOf.call(accounts[3])).toString()], balances, 'List of holders is different than [' + balances.join(', ') + ']');
  });
});
